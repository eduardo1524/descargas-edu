...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        ...
        if cloudtype == 'moodle':
            ...
            if user_info['tokenize']!=0:
               tokenize = True
            bot.editMessageText(message,'⏫Subiendo. ☁ Por favor espere... ')
...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if file_size > max_file_size:
        ...
        file_upload_count = 1
    bot.editMessageText(message,'📦 Preparando el archivo ♻...')
    evidname = ''
    files = []
    if client:
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/ban' in msgText:
            ...
            if isadmin:
                try:
                    user = str(msgText).split(' ')[1]
                    if user == username:
                        bot.sendMessage(update.message.chat.id,'⚠ Este usuario es el Administrador/Desarrollador ⚠')
                        return
                    jdb.remove(user)
                    jdb.save()
                    msg = '🚷 @'+user+' ⚠ Baneado ⚠'
                    ...
                    bot.sendMessage(update.message.chat.id,f'⚠ Error al usar el comando /ban user ⚠')
            else:
                bot.sendMessage(update.message.chat.id,'⚠ No tienes acceso a este comando ⚠')
            return
        if '/db' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
        ...
        if '/about' in msgText:
            bot.sendMessage(update.message.chat.id, f'🤖 Versión del código: 7.1')
            return
        ...
        if '/search_proxy' in msgText:
            msg_start = 'Buscando un proxy, esto puede taradar varias horas.'
        ...
        if '/my' in msgText:
            ...
            if preview:
                bot.sendMessage(update.message.chat.id, f'⚠ No puede ver el usuario de la nube ⚠')
            else:
                getUser = user_info
        ...
        if '/zip' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save()
                   msg = '🗜 El tamaño actual de los zips es de '+ sizeof_fmt(size*1024*1024) +'\n'
                   msg+= 'Añadiendo configuración. Puede tomar unos segundos.\n'
                   bot.sendMessage(update.message.chat.id,msg)
                   statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
        ...
        if '/uptype' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠ Error en el comando /uptype (tipo de subida: evidence, draft, blog, calendar, perfil) ⚠')
            return
        if '/proxy' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            ...
            msg+= '🍜 Desarrollado por @Walkercito para uso personal.'
...
def main():
    ...
    #set in debug
    bot_token = '5546206042:AAF7_Ny2PGVvtJVB8SAVuAaY4IQzf0BeN1I'
